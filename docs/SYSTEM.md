...
- **Host** — GitHub Pages (`aivara-se/bot-mimi`)
...
Until the custom domain answers, the site is reachable at `https://aivara-se.github.io/bot-mimi/`.
...
|---|---|---|---|
| `CNAME` | `mimi` | `aivara-se.github.io` | **DNS only (grey cloud)** |
...
Each site shows `mimi@aivara.se` (and so on) in a `mailto:` link. **The repository has no part in delivering mail** — there is no form, no endpoint, no backend. The address only works once the owner has configured forwarding for `aivara.se` to a mailbox someone reads, so set that up before the site goes public.
...
curl -sI https://mimi.aivara.se | head -1
curl -s  https://mimi.aivara.se | grep -o '<title>[^<]*</title>'
curl -s  https://mimi.aivara.se/log.html | grep -c 'ENTRIES:START'   # expect 1
```
...
- **`Enforce HTTPS` greyed out** — certificate not issued yet, or DNS is proxied (orange cloud). Check with `dig +short mimi.aivara.se`: it must return GitHub's addresses, not Cloudflare's.
